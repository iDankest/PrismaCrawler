  ...
  useEffect(() => {
    const fetchMap = async () => {
      try {
        ...
        console.warn("No se pudo cargar el mapa. Generando mapa de fallback...", error.message)
...
// Efecto para inicializar Phaser (Modificado para esperar al mapData)
  useEffect(() => {
    ...
    class CustomGameScene extends GameScene {
      create() {
        ...
        // --- NUEVO: Enviamos el mapa descargado a la lógica de Phaser ---
        // Asumimos que vas a crear un método llamado "setupBackendMap" en gameScene.js
  ...
  }, [isLoading, mapData]) // --- IMPORTANTE: Ahora depende de la carga del mapa ---
    ...
    <div className="relative w-full h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-blue-900 flex items-center justify-center">
      ...
      <div className="relative">
        <div 
          ...
          style={{
            boxShadow: '0 0 30px rgba(6, 182, 212, 0.4), inset 0 0 20px rgba(59, 130, 246, 0.1)'
          }}
        />
      </div>
      ...
      {gameOver && finalStats && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-gradient-to-br from-slate-800 to-slate-900 border-2 border-cyan-500 rounded-xl p-8 max-w-md w-full mx-4"
            style={{ boxShadow: '0 0 40px rgba(6, 182, 212, 0.5), inset 0 0 30px rgba(59, 130, 246, 0.1)' }}>
            
            <h1 className="text-4xl font-black text-center text-red-500 mb-8 animate-pulse"
            ...
            <div className="space-y-4 mb-8">
              <div className="flex justify-between items-center p-3 bg-slate-900/50 border-l-4 border-cyan-500 rounded hover:bg-slate-900/70 transition-colors">
                <span className="text-slate-300 font-mono text-sm">Floor Reached</span>
                <span className="text-cyan-400 font-mono font-bold text-lg">{finalStats.floor}</span>
              </div>

              <div className="flex justify-between items-center p-3 bg-slate-900/50 border-l-4 border-cyan-500 rounded hover:bg-slate-900/70 transition-colors">
                <span className="text-slate-300 font-mono text-sm">Enemies Slain</span>
                <span className="text-cyan-400 font-mono font-bold text-lg">{finalStats.kills}</span>
              </div>

              <div className="flex justify-between items-center p-3 bg-slate-900/50 border-l-4 border-cyan-500 rounded hover:bg-slate-900/70 transition-colors">
                <span className="text-slate-300 font-mono text-sm">Crystals Found</span>
                <span className="text-cyan-400 font-mono font-bold text-lg">{finalStats.money}</span>
              </div>

              <div className="flex justify-between items-center p-3 bg-slate-900/50 border-l-4 border-cyan-500 rounded hover:bg-slate-900/70 transition-colors">
                <span className="text-slate-300 font-mono text-sm">Items Collected</span>
                <span className="text-cyan-400 font-mono font-bold text-lg">{finalStats.itemsCollected}</span>
              </div>
            </div>
