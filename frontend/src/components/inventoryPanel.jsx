...
import { getItemBySpriteKey } from '../data/itemsDatabase'

export function InventoryPanel({ inventory = [] }) {
  // ✅ Convertir IDs a objetos completos
  const inventoryItems = inventory.map(itemId => {
    return getItemBySpriteKey(itemId)
  }).filter(Boolean) // Filtrar items inválidos
    ...
    <div className="absolute left-4 bottom-4 z-50 flex flex-col gap-2">
      ...
      <div className="grid grid-cols-8 gap-1 bg-slate-900/40 p-2 rounded-lg border border-white/5 backdrop-blur-sm">
        {inventoryItems.map((item, index) => (
          <div 
            key={index}
            title={item?.name || 'Unknown'} // Al pasar el ratón se ve el nombre
