...
export class GameScene extends Phaser.Scene {
  ...
  createDungeonObstacles() {
    ...
    obstaclePositions.forEach(pos => {
      ...
      const crystal = this.add.rectangle(pos.x, pos.y, pos.w - 4, pos.h - 4, 0x3b82f6)
  ...
  checkObstacleCollision(x, y, radius = 20) {
    for (let obstacle of this.obstacles) {
      const closestX = Phaser.Math.Clamp(x, obstacle.x - obstacle.width / 2 - radius, obstacle.x + obstacle.width / 2 + radius)
